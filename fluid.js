// WebGL Fluid Simulation - Converted from React SplashCursor
(function() {
    'use strict';
    ...
    function getSupportedFormat(internalFormat, format, type) {
        if (!supportRenderTextureFormat(internalFormat, format, type)) {
            if (internalFormat === gl.R16F || internalFormat === gl.RED) {
                return getSupportedFormat(gl.RG16F || gl.RG, gl.RG, type);
            } else if (internalFormat === gl.RG16F || internalFormat === gl.RG) {
                return getSupportedFormat(gl.RGBA16F || gl.RGBA, gl.RGBA, type);
            }
            return null;
        }
    ...
    const copyProgram = new Program(baseVertex, compileShader(gl.FRAGMENT_SHADER, copyShader));
    ...
    const advectionProgram = new Program(baseVertex, compileShader(gl.FRAGMENT_SHADER, advectionShader, supportLinearFiltering ? null : ['MANUAL_FILTERING']));
    ...
    const gradienSubtractProgram = new Program(baseVertex, compileShader(gl.FRAGMENT_SHADER, gradientSubtractShader));
    const displayMaterial = new Program(baseVertex, compileShader(gl.FRAGMENT_SHADER, displayShaderSource));
    ...
    function initFramebuffers() {
        ...
        const texType2 = halfFloatTexType;
        const rgba2 = rgba;
        const rg2 = rg;
        const r2 = r;
        const filtering2 = filtering;

        dye = createDoubleFBO(dyeRes.width, dyeRes.height, rgba2.internalFormat, rgba2.format, texType2, filtering2);
        velocity = createDoubleFBO(simRes.width, simRes.height, rg2.internalFormat, rg2.format, texType2, filtering2);
        divergence = createFBO(simRes.width, simRes.height, r2.internalFormat, r2.format, texType2, gl.NEAREST);
        curl = createFBO(simRes.width, simRes.height, r2.internalFormat, r2.format, texType2, gl.NEAREST);
        pressure = createDoubleFBO(simRes.width, simRes.height, r2.internalFormat, r2.format, texType2, gl.NEAREST);
    }
    ...
    window.addEventListener('mousemove', e => {
        ...
        if (!pointer.down) updatePointerDownData(pointer, -1, e.clientX, e.clientY);
    ...
    window.addEventListener('touchmove', e => {
        ...
        for (let i = 0; i < touches.length; i++) {
            ...
            if (pointer) updatePointerMoveData(pointer, touches[i].clientX, touches[i].clientY);
        }
    }, { passive: true });
